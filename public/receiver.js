document.addEventListener('DOMContentLoaded', () => {
  ...
  function handleDataMessage(e) {
    if (typeof e.data === 'string') {
      ...
      if (msg.type === 'header') startFile(msg.fileId, msg.meta);
        ...
        const { fileId, chunkIndex, chunk } = msg;
        const fileObj = filesInProgress.get(fileId);
        if (!fileObj) return;

        fileObj.buffer[chunkIndex] = new Uint8Array(chunk);
        fileObj.received += chunk.byteLength;
        updateProgress(fileObj);

        // Send ACK for this chunk
        dataChannel.send(JSON.stringify({ type: 'ack', fileId, chunkIndex }));
  ...
  function startFile(fileId, meta) {
    ...
    filesInProgress.set(fileId, { meta, buffer: [], received: 0, row, doneReceived: false });
  ...
  function finishFile(fileId) {
    ...
    const blob = new Blob(fileObj.buffer, { type: fileObj.meta.type || 'application/octet-stream' });
